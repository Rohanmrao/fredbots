...
def handle_tasker(req):
    ...
    try:
        ...
        pickup_x, pickup_y = -1, -1
        destination_x, destination_y = -1, -1
    ...
    for robot in robots.values():
        robot_table.append([
            ...
            robot.task_positions[0] if robot.task_positions[0] is not (-1, -1) else "No task assigned",
            robot.task_positions[1] if robot.task_positions[1] is not (-1, -1) else "No task assigned",
        ])
...
def main():
    global robots, packages, package_number, first_time
    packages = []
    package_number = 0
    first_time = True
