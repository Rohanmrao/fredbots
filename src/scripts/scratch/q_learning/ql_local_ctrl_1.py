...
goal_x = 4  # int(input("Please enter goal x coordinate: "))
goal_y = 2  # int(input("Please enter goal y coordinate: "))
...
def get_next_action_random(current_row_index, current_column_index, action_index):
    act = np.random.randint(4)

    while act == action_index:
       act = np.random.randint(4)
  ...
  if is_final_state(start_row_index, start_column_index):
    ...
    while not is_final_state(current_row_index, current_column_index):
        ...
        if flag:
            ...
            next_row_index, next_column_index = get_next_location(current_row_index, current_column_index, action_index)
    ...
    # *****************************************************************
        if (response.occ == 0): # [NOT OCCUPIED]
            ...
            if atom_sim:
               print("Moving to location: ", next_row_index, next_column_index)
    ...
    # *****************************************************************
            ...
            flag = False
            action_index = get_next_action_random(current_row_index, current_column_index, action_index)
            next_row_index, next_column_index = get_next_location(current_row_index, current_column_index, action_index)
            print("\n\n")
        ...
        current_row_index, current_column_index = next_row_index, next_column_index
        shortest_path.append([next_row_index, next_column_index ])
        global obstacles
        obstacles = []
    print("\n\n")
