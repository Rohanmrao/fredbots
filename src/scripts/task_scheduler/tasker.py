...
def assign_tasks(robots, packages):
    for package in packages:
        if not package.picked_up:
            # max_distance is hte bot very far from that package
            ...
            print("package utility dict for all robots sorted: ", package.utilities)
...
# handle_tasker function to handle incoming service requests
def handle_tasker(req):
    ...
    print("req hat we get from services: ", robot_id, x_pos, y_pos)
...
def main():
    ...
    # package_id, pickup_position, dropoff_position, priority, weight

    # sort packages by priority
    packages.sort(key=lambda x: x.priority, reverse=True)

    print("priority order of packages: ", packages)
    ...
    # robot_id, intital_position of the robots

    tasker_server() 

    # tasker_server() is a function that sets up a ROS service server to handle task assignments for the robots
